...
def create_quiz(self, file_names: list[str], pk: int, max_questions: Union[int, None] = None,
    ...
    try:
        for temp_quiz, i, n in quiz_gen.create_quiz_from_files(
            ...
            **{
            ...
            meta = {'current': i, 'total': n}  # Meta data of generation process
    ...
    if description:
        ml_quiz.set_description(description)  # Setting description in vector database
    else:
        describer = QuizDescriber()  # Initializing quiz description generating
        ml_quiz = describer.generate_description(ml_quiz)  # Updating quiz with description
        quiz.description = ml_quiz.description  # Updating description for the quiz
    quiz.created_at = datetime.datetime.now()  # Setting creation time of the quiz
    quiz.ready = True  # Setting the quiz to ready state
    quiz.save()  # Saving quiz to database
    SEARCH_DB.save_quiz(quiz=ml_quiz, unique_id=str(quiz.id))  # Saving quiz to vector database
    print(f'Quiz {pk} was created successfully')  # Printing message for logging
    return meta
