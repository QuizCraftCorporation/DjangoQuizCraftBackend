...
class KeyWord(models.Model):
    ...
    name = models.CharField(
        _("key word"), primary_key=True, unique=True, max_length=30
    )
...
class Material(models.Model):
    ...
    name = models.CharField(
        _("material name"), max_length=60, null=False, blank=True
    )
    id = models.BigAutoField(verbose_name="material id", primary_key=True)
    file = models.FileField(upload_to="materials/")
    topic = models.ForeignKey(
        to=Topic, on_delete=models.PROTECT, verbose_name="topic", null=True
    )
        ...
        for ml_question in model_questions:
            ...
            for option in ml_question.options:
                ...
                mcq_option = MCQOption(
                    text=option, correct=correct, question=mcq
                )
                mcq_option.save()
...
class QuizView(models.Model):
    ...
    viewer = models.ForeignKey(
        User, on_delete=models.PROTECT, related_name="viewed_quizzes"
    )
...
class QuizGroup(models.Model):
    ...
    name = models.CharField(
        _("group name"), max_length=42, null=False, blank=False
    )
...
class Question(models.Model):
    ...
    type_id = models.PositiveSmallIntegerField(
        default=1, choices=QUESTION_TYPE
    )
...
class AbstractQuestion(models.Model):
    ...
    The `get_answer()` method should return the answer for the current
    question, and the `get_evaluator()` method should return a class
    that can be used to evaluate the answer to the current question.
...
class MCQQuestion(AbstractQuestion):
    ...
    answer to the question by checking whether it is one
    ...
    def get_answer(self):
        ...
        """
        question_answers = {
            option.id for option in self.options.filter(correct__exact=True)
        }
...
class TrueFalseQuestion(AbstractQuestion):
    ...
    The `get_evaluator()` method returns a class that can evaluate a user's
    answer to the question by checking whether it is the same
...
class InsertionPosition(models.Model):
    ...
    def __str__(self):
        return (
            f"InsertionPosition(question={self.question}, "
            f"position={self.position})"
        )

    question = models.ForeignKey(
        to="InsertionQuestion",
        verbose_name="question id",
        null=False,
        on_delete=models.CASCADE,
    )
    position = models.PositiveIntegerField(
        verbose_name="position offset", null=False
    )
...
class InsertionQuestion(AbstractQuestion):
    ...
    def __str__(self):
        return (
            f"InsertionQuestion(question={self.question}, "
            f"insertion_text={self.insertion_text})"
        )
    ...
    def get_answer(self):
        ...
        """
        answers = [
            insertion_answer.answer
            for insertion_answer in map(
                lambda insertion_answer: insertion_answer.answer,
                sorted(self.insertion_answers.all(), key=lambda x: x.position),
            )
        ]
        return answers
...
class InsertionAnswer(models.Model):
    ...
    def __str__(self):
        return (
            f"InsertionAnswer(question={self.question}, answer={self.answer}, "
            f"position={self.position})"
        )
...
class Take(models.Model):
    ...
    def __str__(self):
        return (
            f"Take(quiz={self.quiz}, user={self.user}, points={self.points}, "
            f"passage_date={self.passage_date})"
        )

    quiz = models.ForeignKey(
        to=Quiz, on_delete=models.PROTECT, verbose_name="quiz id"
    )
    user = models.ForeignKey(
        to=User, on_delete=models.PROTECT, verbose_name="user id"
    )
    points = models.FloatField(_("points"))
    passage_date = models.DateField(
        _("passage date"), auto_now_add=True, null=False
    )
    id = models.BigAutoField(
        _("take id"), primary_key=True, auto_created=True, db_index=True
    )
